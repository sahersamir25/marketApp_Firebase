...
class ItemViewVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if item != nil {
            ...
            priceLbl.text = convertToCurrency(item.price ?? 0)
            descriptionText.text = item.description
        }
        images()
        
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "backward.end"), style: .plain, target: self, action: #selector(self.backAction))
        
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "cart"), style: .plain, target: self, action: #selector(self.basketAction))
